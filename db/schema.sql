...
-- ============================================================================
-- 7. ingest_requests — POST /ingest 멱등성
-- ============================================================================
-- 같은 요청이 두 번 도착하는 경로가 실재한다. 기사는 INSERT OR IGNORE 로
-- 안전하지만 sources 의 헬스 상태는 부작용이 남는다 — 응답이 유실돼 재시도되면
-- 전건 duplicated 가 되어 empty_streak 이 잘못 +1 되고, 살아 있는 피드가
-- 죽은 것으로 집계된다.
--
...
CREATE TABLE ingest_requests (
    ...
    source_id     INTEGER NOT NULL REFERENCES sources(id),
    -- 같은 request_id 로 다른 내용이 오는 사고를 막는다. 해시가 다르면
    -- 저장된 응답을 돌려주는 게 아니라 409 다 — 옛 응답을 돌려주면
    -- 조용히 틀린 답이 된다
...
-- 보존 기간 정리용. 기간은 n8n 의 실제 재시도 범위를 보고 정한다 —
